...
const BookCart = () => {
    ...
    const fetchCartData = () => {
      axios.get(`http://localhost:8083/CartPage/Show_Cart_Record?token=${localStorage.getItem("AccessToken")}`)
      .then((res) => {
    ...
    const increaseBookQty= (bookId) => {
      axios.put(`http://localhost:8083/CartPage/IncreaseBookQty?bookId=${bookId}&token=${localStorage.getItem("AccessToken")}`)
      .then((res) => {
    ...
    const decreaseBookQty= (bookId) => {
      axios.put(`http://localhost:8083/CartPage/DecreaseBookQty?bookId=${bookId}&token=${localStorage.getItem("AccessToken")}`)
      .then((res) => {
        toast.success(res.data.message, {position: toast.POSITION.BOTTOM_CENTER});
        fetchCartData();
      })
    ...
    const removeBookFromCart=(id) => {
      axios.delete(`http://localhost:8083/CartPage/Remove_Book_From_Cart?cartBookId=${id}&token=${localStorage.getItem("AccessToken")}`)
      .then((res) => {
        console.log(res.data);
        toast.success(res.data.message, {position: toast.POSITION.BOTTOM_CENTER});
        fetchCartData();
